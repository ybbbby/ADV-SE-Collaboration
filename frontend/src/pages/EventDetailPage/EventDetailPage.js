...
export default function EventDetail(props) {
  ...
  const clickRegister = (val) => {
    if (!user) {
      setLoginOpen(true)
      return
    }
    const url = 'user/event/' + router.match.params.eventID.toString() + 'join'

    const requestForm = new FormData()
    requestForm.append('join', val)
    fetch(url, {
      method: 'POST',
      body: requestForm,
    })
      ...
      .catch((error) => {
        setfailInfo('Cannot connect to server')
        setServerity('error')
        setAlertOpen(true)
      })
  }
